...
class SMExtendWallViewProvider(SMViewProvider):
  def getIcon(self):
    return self.getIconPath() + "SMExtend.svg"
    

class SMExtendCommandClass():
  """Extend face"""

  def GetResources(self):
    return {'Pixmap'  : SMViewProvider.getIconPath() + 'SMExtend.svg', # the name of a svg file available in the resources
  ...
  def Activated(self):
    ...
    a = SMFeature.GetPythonObj("Extend")
    SMExtendWall(a)
    SMExtendWallViewProvider(a.ViewObject)
    doc.recompute()
    doc.commitTransaction()
    return
...
Gui.addCommand('SMExtendFace',SMExtendCommandClass())
